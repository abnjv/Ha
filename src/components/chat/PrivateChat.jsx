...
const PrivateChat = () => {
  ...
  const sendDbMessage = async (messageData) => { await addDoc(collection(db, getPrivateChatMessagesPath(appId, chatPartnersId)), { ...messageData, senderId: user.uid, createdAt: serverTimestamp(), isEdited: false, replyTo: replyingToMessage ? { id: replyingToMessage.id, text: replyingToMessage.text, senderId: replyingToMessage.senderId, senderName: messages.find(m => m.senderId === replyingToMessage.senderId)?.senderName || 'Unknown' } : null }); setReplyingToMessage(null); };
  ...
  return (
    <div className={`relative flex flex-col min-h-screen p-4 antialiased ${themeClasses}`}>
      ...
      <header className={`flex items-center space-x-2 p-4 rounded-3xl mb-4 shadow-lg ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
        ...
        <button onClick={() => !isChatWithBot && setIsProfileModalOpen(true)} className="text-2xl font-extrabold flex-1 text-left hover:underline" disabled={isChatWithBot}>{friendName}</button>
